...
// Rules
function getNextState(board: Cell[][]): void {
  const cellsToChange: { [key: string]: boolean } = {};
  for (let row = 0; row < board.length; row++) {
    for (let col = 0; col < board[row].length; col++) {
      let count = 0;
      if (board[row - 1]) {
        if (board[row - 1][col - 1] && !board[row - 1][col - 1].isDead) count++;
        if (board[row - 1][col] && !board[row - 1][col].isDead) count++;
        if (board[row - 1][col + 1] && !board[row - 1][col + 1].isDead) count++;
      }
      if (board[row][col - 1] && !board[row][col - 1].isDead) count++;
      if (board[row][col + 1] && !board[row][col + 1].isDead) count++;
      if (board[row + 1]) {
        if (board[row + 1][col - 1] && !board[row + 1][col - 1].isDead) count++;
        if (board[row + 1][col] && !board[row + 1][col].isDead) count++;
